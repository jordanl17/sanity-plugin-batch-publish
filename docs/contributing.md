...
```
feat(plugin): add batch publish action
...
`sanity-plugin-batch-publish` uses `@sanity/pkg-utils --strict`, which runs api-extractor. Every public export **must** have a `/** @public */` JSDoc tag; missing tags fail the build.
...
```
packages/sanity-plugin-batch-publish/src/__tests__/plugin.test.ts
```
